...
import { listNotificationsRequest, markAllNotificationsReadRequest, markNotificationReadRequest } from '@/features/chat/api';
...
export function NotificationsPage() {
  ...
  useEffect(() => {
    ...
    listNotificationsRequest()
      ...
      .catch((cause) => {
        if (!cancelled) setError(cause instanceof Error ? cause.message : 'Unable to load notifications.');
      })
      .finally(() => {
    ...
    const unsubscribe = subscribeChatSocket({
      onNotification(notification) {
        ...
        setItems((current) => [notification, ...current.filter((row) => row.id !== notification.id)]);
      },
    });
    return () => {
      cancelled = true;
    ...
    <div className="space-y-6">
      <header className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">Mentions, messages, reactions, meetings, and daily work.</p>
        </div>
      ...
      <div className="divide-y divide-line overflow-hidden rounded-lg border border-line bg-surface">
        {items.length === 0 && !loading ? (
          <EmptyState title="No notifications yet" description="Mentions, messages, and reminders will show up here." />
        ) : null}
        {items.map((item) => (
          <button
            key={item.id}
            ...
            onClick={() => {
              ...
              if (item.type === NOTIFICATION_TYPE.MEETING_CREATED || item.type === NOTIFICATION_TYPE.MEETING_REMINDER) {
                navigate('/meetings');
                return;
              }
              if (item.referenceId) {
          ...
          >
            <p className={`text-sm ${item.isRead ? 'text-muted' : 'font-medium text-ink'}`}>{item.title}</p>
