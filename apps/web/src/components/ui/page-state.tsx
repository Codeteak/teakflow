...
export function ErrorBanner({ message, className }: { message: string; className?: string }) {
  ...
  return (
    <p
      role="alert"
      className={cn('rounded-md border border-rose/30 bg-rose/5 px-3 py-2 text-sm text-rose', className)}
    >
      {message}
    </p>
  );
  ...
  return (
    <div className={cn('space-y-3', className)} aria-busy="true" aria-label="Loading">
      {Array.from({ length: rows }, (_, index) => (
        <div key={index} className="space-y-2 rounded-lg border border-line bg-surface p-4">
...
export function ListLoading({ rows = 5, className }: { rows?: number; className?: string }) {
  return (
    <div className={cn('space-y-1 px-2', className)} aria-busy="true" aria-label="Loading">
      {Array.from({ length: rows }, (_, index) => (
